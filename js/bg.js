	...
	function init() {
		...
		for ( var i = 0; i < 100; i ++ ) {
			...
			particle.position.x = Math.random() * 900 - 600;
			particle.position.y = Math.random() * 900 - 600;
			particle.position.z = Math.random() * 900 - 600;
			particle.scale.x = particle.scale.y = Math.random() * 10 + 20;
			scene.add( particle );
